...
#+begin_src sh :tangle ~/bin/list-generations :shebang #!/bin/sh
  ...
  profile=$1
  guix package --profile=$HOME/.guix-extra-profiles/$profile/$profile --list-generations
...
#+begin_src sh :tangle ~/bin/delete-generations :shebang #!/bin/sh
  ...
  profile=$1
  generations=$2
  echo Deleting generations $generations of $profile
  guix package --profile=$HOME/.guix-extra-profiles/$profile/$profile --delete-generations=$generations
...
#+begin_src sh :tangle ~/bin/update-profiles :shebang #!/bin/sh
  ...
  for profile in $profiles; do
    ...
    guix package --profile="$profilePath/$profileName" --manifest="$HOME/.config/guix/manifests/$profileName.scm" --fallback
  done
...
#+begin_src sh :tangle ~/bin/update-system :shebang #!/usr/bin/env sh
  ...
  sudo guix system -L ~/.config/guix --fallback reconfigure ~/.config/guix/systems/$HOSTNAME.scm
